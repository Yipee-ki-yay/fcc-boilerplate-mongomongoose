...
const createAndSavePerson = (done) => {
  const person = new Person({ name: 'Frodo', age: 40, favoriteFoods: ['lavash']});
...
const createManyPeople = (arrayOfPeople, done) => {
  const people = arrayOfPeople.map(({name, age, favoriteFoods}) => new Person({ name, age, favoriteFoods}));
...
const removeById = (personId, done) => {
  Person.findByIdAndRemove(personId, function(err, person) {
    person.save(function(err, deletedPerson) {
